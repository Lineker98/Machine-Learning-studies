...
#Transformatios of predictor atributes, we will code categoricals atributes in discreet atributes
...
# We'll make our sequential neural networks with two hidden layers, that each hidden layers, with eight perceptrons, will be acticavated
# by relu function, the nn will have fourteen inputs and only one output, being the output activated by sigmoid function
classifier = Sequential()
classifier.add(Dense(units=8, activation='relu', input_dim=14))
classifier.add(Dense(units=8, activation='relu'))
classifier.add(Dense(units=1, activation='sigmoid'))
classifier.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy'])
classifier.fit(predictors_training, classe_training, batch_size=10, epochs=100)
